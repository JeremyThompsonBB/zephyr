...
/** Blocking register read via @p bus->i2c_spec (sub-address write + read). */
int bmp581_reg_read_rtio(const struct bmp581_bus *bus, uint8_t start, uint8_t *buf, int size);

/** Blocking register write via @p bus->i2c_spec (reg byte + payload, payload up to 8 bytes). */
int bmp581_reg_write_rtio(const struct bmp581_bus *bus, uint8_t reg, const uint8_t *buf, int size);

/** Blocking burst write (len <= 8), e.g. soft reset. */
