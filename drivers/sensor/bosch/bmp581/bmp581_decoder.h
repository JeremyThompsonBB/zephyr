...
static inline enum bmp581_event
bmp581_encode_events_bitmask(const struct sensor_stream_trigger *const triggers, size_t count)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		case SENSOR_TRIG_DATA_READY:
			result |=
				((triggers[i].opt == SENSOR_STREAM_DATA_INCLUDE) ? BMP581_EVENT_DRDY
										 : 0);
			break;
		case SENSOR_TRIG_FIFO_WATERMARK:
			result |= ((triggers[i].opt == SENSOR_STREAM_DATA_INCLUDE)
					   ? BMP581_EVENT_FIFO_WM
					   : 0);
			break;
		default:
			break;
